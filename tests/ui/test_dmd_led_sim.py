...
class TestApplyLedGrid(unittest.TestCase):
    ...
    def test_first_column_of_each_cell_is_lit(self):
        scale, gap = 4, 1
        img = self._white_img(16, 4)
        out = apply_led_grid(img, scale, gap)
        arr = np.array(out)
        for col in range(0, 16, scale):
            self.assertFalse(np.all(arr[:, col] == 0),
                            f"Column {col} should NOT be black")
    ...
    def test_first_row_of_each_cell_is_lit(self):
        scale, gap = 4, 1
        img = self._white_img(4, 16)
        out = apply_led_grid(img, scale, gap)
        arr = np.array(out)
        for row in range(0, 16, scale):
            self.assertFalse(np.all(arr[row, :] == 0),
                            f"Row {row} should NOT be black")
