...
class SettingsPanel(ctk.CTkFrame):
    ...
    def _build_params_panel(self, parent):
        ...
        self._per_gif_cb = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_per_gif_config,
        ...
        self._auto_color_cb = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_auto_color_enabled,
        ...
        # Parallelism
        section("⚡  Parallelism")
        slider_row("Workers (CPU)", self.app_state.v_workers, 1, 16, "{:.0f}", " workers", steps=15, lmh=False)

        # Scroll
        section("📜  Scroll")
        slider_row("Scroll speed",    self.app_state.v_scroll,        4.0, 80.0, "{:.0f}", " px/s")
        ...
        slider_row("FPS maximum", self.app_state.v_fps_max, 10.0, 60.0, "{:.1f}", " fps") # FIX: Passed self.app_state.v_fps_max as var
        ...
        self._max_dur_cb = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_max_dur_enabled,
        ...
        self._max_dur_slider = ctk.CTkSlider(
            dur_row, from_=10, to=600, variable=self.app_state.v_max_duration,
            number_of_steps=118,
        )
        ...
        self._max_dur_lbl = ctk.CTkLabel(
            dur_row, text=_fmt_dur(self.app_state.v_max_duration.get()),
            width=72, anchor="e", font=ctk.CTkFont(size=11)
        )
        self._max_dur_lbl.grid(row=0, column=2, padx=(4, 4))
        self.app_state.v_max_duration.trace_add(
            "write",
            lambda *_: (
                self._max_dur_lbl.configure(text=_fmt_dur(self.app_state.v_max_duration.get())),
        ...
        ctk.CTkEntry(api_frame, textvariable=self.app_state.v_tenor_api_key, show="*").grid(row=0, column=1, sticky="ew", padx=4, pady=2)
        
        ctk.CTkLabel(api_frame, text="Giphy API Key", width=100, anchor="w",
                     font=ctk.CTkFont(size=12)).grid(row=1, column=0, padx=(4, 6), pady=2)
        ctk.CTkEntry(api_frame, textvariable=self.app_state.v_giphy_api_key, show="*").grid(row=1, column=1, sticky="ew", padx=4, pady=2)
        ...
        cslider("Contrast",    self.app_state.v_contrast,    0.5,  2.5)
        cslider("Saturation",  self.app_state.v_saturation,  0.0,  4.0)
        cslider("Brightness",  self.app_state.v_brightness, -0.5,  0.5, "{:.3f}")
        cslider("Gamma",       self.app_state.v_gamma,       0.1,  2.5)
        cslider("Sharpen Lum", self.app_state.v_sharpen_lum, 0.0,  3.0)
        cslider("Sharpen Chr", self.app_state.v_sharpen_chr, 0.0,  2.0)
        ...
        self._dither_menu = ctk.CTkOptionMenu(
            dr, variable=self.app_state.v_dither,
    ...
    def _build_advanced_content(self, parent):
        ...
        self._cb_auto_action_enabled = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_auto_action_enabled,
            font=ctk.CTkFont(size=12), text_color="#aaddaa",
        )
        ...
        self._cb_dmd_visibility_score_enabled = ctk.CTkCheckBox(
            row_frame,
            text="DMD Visibilité",
            variable=self.app_state.v_dmd_visibility_score_enabled,
            font=ctk.CTkFont(size=12), text_color="#aaddaa",
        )
        ...
        self._cb_dmd_readability_score_enabled = ctk.CTkCheckBox(
            row_frame,
            text="DMD Lisibilité",
            variable=self.app_state.v_dmd_readability_score_enabled,
            font=ctk.CTkFont(size=12), text_color="#aaddaa",
        )
        ...
        adv_slider(parent, "ROI padding", self.app_state.v_action_padding, 0.0, 0.6,
                   "{:.2f}", "", steps=60)
        adv_slider(parent, "Intro panoramic", self.app_state.v_action_intro, 0.0, 5.0,
                   "{:.1f}", " s", steps=50)
        ...
        self._cb_auto_pillarbox = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_action_auto_pillarbox,
            font=ctk.CTkFont(size=12), text_color="#ffe08a",
        )
        ...
        self._slider_bottom_crop = adv_slider(parent, "Bottom crop (%)", self.app_state.v_action_bottom_crop, 0.0, 0.5,
                   "{:.0%}", "", steps=50)
        ...
        def _toggle_auto_bottom_crop(*_):
            if self.app_state.v_action_smart_auto_crop.get():
        ...
        self._slider_top_crop = adv_slider(parent, "Top crop (%)", self.app_state.v_action_top_crop, 0.0, 0.5,
                   "{:.0%}", "", steps=50)
        ...
        self._cb_bg_sub = ctk.CTkCheckBox(
            ...
            variable=self.app_state.v_bg_sub_enable,
            font=ctk.CTkFont(size=12), text_color="#aaddaa",
        )
        ...
        self._on_target_preset_change(self.app_state.v_target_preset.get()) # Set initial state
    ...
    def _on_let_me_handle_toggle(self):
        ...
        if enabled:
            ...
            self._lmh_saved_state = {
                "auto_color_enabled":    self.app_state.v_auto_color_enabled.get(),
                "auto_action_enabled":   self.app_state.v_auto_action_enabled.get(),
                "action_smart_auto_crop": self.app_state.v_action_smart_auto_crop.get(),
                "bg_sub_enable":         self.app_state.v_bg_sub_enable.get(),
                "dmd_visibility_score_enabled": self.app_state.v_dmd_visibility_score_enabled.get(),
                "dmd_readability_score_enabled": self.app_state.v_dmd_readability_score_enabled.get(),
                "action_auto_strength": self.app_state.v_action_auto_strength.get(),
                "action_auto_smoothness": self.app_state.v_action_auto_smoothness.get(),
                "action_auto_pillarbox": self.app_state.v_action_auto_pillarbox.get(),
            }
            # Force all flags ON (visual feedback)
            self.app_state.v_auto_color_enabled.set(True)
            self.app_state.v_auto_action_enabled.set(True)
            self.app_state.v_action_smart_auto_crop.set(True)
            self.app_state.v_dmd_visibility_score_enabled.set(True)
            self.app_state.v_dmd_readability_score_enabled.set(True)
            self.app_state.v_action_auto_strength.set(True)
            self.app_state.v_action_auto_smoothness.set(True)
            self.app_state.v_action_auto_pillarbox.set(True)
            ...
            self.app_state.v_auto_color_enabled.set(saved.get("auto_color_enabled", False))
            self.app_state.v_auto_action_enabled.set(saved.get("auto_action_enabled", False))
            self.app_state.v_action_smart_auto_crop.set(saved.get("action_smart_auto_crop", False))
            self.app_state.v_bg_sub_enable.set(saved.get("bg_sub_enable", False))
            self.app_state.v_dmd_visibility_score_enabled.set(saved.get("dmd_visibility_score_enabled", False))
            self.app_state.v_dmd_readability_score_enabled.set(saved.get("dmd_readability_score_enabled", True))
            self.app_state.v_action_auto_strength.set(saved.get("action_auto_strength", False))
            self.app_state.v_action_auto_smoothness.set(saved.get("action_auto_smoothness", False))
            self.app_state.v_action_auto_pillarbox.set(saved.get("action_auto_pillarbox", False))
    ...
    def _reset_advanced(self):
        self.app_state.v_auto_action_enabled.set(False)
        self.app_state.v_action_detector.set("person")
        self.app_state.v_action_strength.set(0.65)
        self.app_state.v_action_auto_strength.set(False)
        self.app_state.v_action_smoothness.set(0.65)
        self.app_state.v_action_auto_smoothness.set(False)
        self.app_state.v_action_zoom_max.set(2.0)
        self.app_state.v_action_padding.set(0.20)
        self.app_state.v_action_intro.set(1.5)
        self.app_state.v_action_bottom_crop.set(0.0)
        self.app_state.v_action_auto_bottom_crop.set(False)
        self.app_state.v_action_top_crop.set(0.0)
        self.app_state.v_action_auto_top_crop.set(False)
        self.app_state.v_action_vertical_bias.set(0.0)
        self.app_state.v_action_auto_vertical_bias.set(False)
        self.app_state.v_action_smart_auto_crop.set(False)
        self.app_state.v_bg_sub_enable.set(False) # Reset background subtraction
        self.app_state.v_dmd_visibility_score_enabled.set(False) # NEW: Reset DMD Visibility Score
        self.app_state.v_dmd_readability_score_enabled.set(True) # NEW: Reset DMD Readability Score
        self.app_state.v_target_preset.set("128x32 (1x1)") # Reset tiling preset
        self.app_state.v_target_width.set(DEFAULT_PARAMS["target_width"])
        self.app_state.v_target_height.set(DEFAULT_PARAMS["target_height"])
        self._on_target_preset_change(self.app_state.v_target_preset.get()) # Apply preset reset
        self.app_state.v_text_overlay_enabled.set(False) # Reset text overlay
        self.app_state.v_text_content.set("")
        self.app_state.v_text_font_size.set(8)
        self.app_state.v_text_color.set("white")
        self.app_state.v_text_position.set("bottom_center")
        self.app_state.v_text_font_file.set("HelvetiPixel.ttf")
        self.app_state.v_text_style.set("outline")
        self.app_state.v_text_bg.set(False)
        self.app_state.v_text_bg_opacity.set(60)
        self.app_state.v_text_animation.set("none")
        self._on_text_overlay_toggle() # Apply text overlay reset
        self.app_state.v_scroll_enabled.set(True)
        self.app_state.v_zoom.set(1.0)
        self.app_state.v_manual_x.set(0)
        self.app_state.v_manual_y.set(0)
        self.app_state.v_hue_shift.set(0.0)
        self.app_state.v_noise_reduction.set(0.0)
        self.app_state.v_film_grain.set(0)
        self.app_state.v_vignette.set(False)
    ...
    def _snapshot_params(self) -> dict:
        ...
        return {
            ...
            "mode":                       self.app_state.v_mode.get(),
            "workers":                    self.app_state.v_workers.get(),
            "scroll":                     self.app_state.v_scroll.get(),
            "bottom_crop":                self.app_state.v_bottom_crop.get(),
            "top_crop":                   self.app_state.v_top_crop.get(),
            "scroll_cycles":              self.app_state.v_scroll_cycles.get(),
            "fps_min":                    self.app_state.v_fps_min.get(),
            "fps_max":                    self.app_state.v_fps_max.get(),
            "contrast":                   self.app_state.v_contrast.get(),
            "saturation":                 self.app_state.v_saturation.get(),
            "brightness":                 self.app_state.v_brightness.get(),
            "gamma":                      self.app_state.v_gamma.get(),
            "sharpen_lum":                self.app_state.v_sharpen_lum.get(),
            "sharpen_chr":                self.app_state.v_sharpen_chr.get(),
            "dither":                     self.app_state.v_dither.get(),
            "scroll_enabled":             self.app_state.v_scroll_enabled.get(),
            "zoom":                       self.app_state.v_zoom.get(),
            "manual_x":                   self.app_state.v_manual_x.get(),
            "manual_y":                   self.app_state.v_manual_y.get(),
            "hue_shift":                  self.app_state.v_hue_shift.get(),
            "noise_reduction":            self.app_state.v_noise_reduction.get(),
            "film_grain":                 self.app_state.v_film_grain.get(),
            "vignette":                   self.app_state.v_vignette.get(),
            "auto_action_enabled":        self.app_state.v_auto_action_enabled.get(),
            "action_detector":            self.app_state.v_action_detector.get(),
            "action_strength":            self.app_state.v_action_strength.get(),
            "action_auto_strength":       self.app_state.v_action_auto_strength.get(),
            "action_smoothness":          self.app_state.v_action_smoothness.get(),
            "action_auto_smoothness":     self.app_state.v_action_auto_smoothness.get(),
            "action_zoom_max":            self.app_state.v_action_zoom_max.get(),
            "action_padding":             self.app_state.v_action_padding.get(),
            "action_intro":               self.app_state.v_action_intro.get(),
            "action_bottom_crop":         self.app_state.v_action_bottom_crop.get(),
            "action_auto_bottom_crop":    self.app_state.v_action_auto_bottom_crop.get(),
            "action_top_crop":            self.app_state.v_action_top_crop.get(),
            "action_auto_top_crop":       self.app_state.v_action_auto_top_crop.get(),
            "action_vertical_bias":       self.app_state.v_action_vertical_bias.get(),
            "action_auto_vertical_bias":  self.app_state.v_action_auto_vertical_bias.get(),
            "action_smart_auto_crop":     self.app_state.v_action_smart_auto_crop.get(),
            "action_auto_pillarbox":      self.app_state.v_action_auto_pillarbox.get(),
            "bg_sub_enable":              self.app_state.v_bg_sub_enable.get(),
            "dmd_visibility_score_enabled": self.app_state.v_dmd_visibility_score_enabled.get(), # NEW
            "dmd_readability_score_enabled": self.app_state.v_dmd_readability_score_enabled.get(), # NEW
            "target_width":               self.app_state.v_target_width.get(),
            "target_height":              self.app_state.v_target_height.get(),
            "target_preset":              self.app_state.v_target_preset.get(),
            "text_overlay_enabled":       self.app_state.v_text_overlay_enabled.get(),
            "text_content":               self.app_state.v_text_content.get(),
            "text_font_size":             self.app_state.v_text_font_size.get(),
            "text_color":                 self.app_state.v_text_color.get(),
            "text_position":              self.app_state.v_text_position.get(),
            "text_font_file":             self.app_state.v_text_font_file.get(),
            "text_style":                 self.app_state.v_text_style.get(),
            "text_bg":                    self.app_state.v_text_bg.get(),
            "text_bg_opacity":            self.app_state.v_text_bg_opacity.get(),
            "text_animation":             self.app_state.v_text_animation.get(),
            "max_dur_enabled":            self.app_state.v_max_dur_enabled.get(),
            "max_duration":               self.app_state.v_max_duration.get(),
            "auto_color_enabled":         self.app_state.v_auto_color_enabled.get(),
        }
    ...
    def _restore_params(self, s: dict):
        ...
        if hasattr(self, "v_trim_start"):
            ...
            self.app_state.v_trim_start.set(ts if ts is not None else 0.0)
        if hasattr(self, "v_trim_end"):
            te = s.get("v_trim_end")
            self.app_state.v_trim_end.set(te if te is not None else 0.0)
            
        self.app_state.v_mode.set(s.get("mode", "pixel_art"))
        self.app_state.v_workers.set(s.get("workers", 2))
        self.app_state.v_scroll.set(s.get("scroll", 24.0))
        self.app_state.v_bottom_crop.set(s.get("bottom_crop", 0.15))
        self.app_state.v_top_crop.set(s.get("top_crop", 0.0))
        self.app_state.v_scroll_cycles.set(s.get("scroll_cycles", 1.5))
        self.app_state.v_fps_min.set(s.get("fps_min", 10.0))
        self.app_state.v_fps_max.set(s.get("fps_max", 25.0))
        self.app_state.v_contrast.set(s.get("contrast", 1.6))
        self.app_state.v_saturation.set(s.get("saturation", 2.2))
        self.app_state.v_brightness.set(s.get("brightness", -0.03))
        self.app_state.v_gamma.set(s.get("gamma", 0.85))
        self.app_state.v_sharpen_lum.set(s.get("sharpen_lum", 1.8))
        self.app_state.v_sharpen_chr.set(s.get("sharpen_chr", 0.5))
        self.app_state.v_dither.set(s.get("dither", "none"))
        self.app_state.v_scroll_enabled.set(s.get("scroll_enabled", True))
        self.app_state.v_zoom.set(s.get("zoom", 1.0))
        self.app_state.v_manual_x.set(s.get("manual_x", 0))
        self.app_state.v_manual_y.set(s.get("manual_y", 0))
        self.app_state.v_hue_shift.set(s.get("hue_shift", 0.0))
        self.app_state.v_noise_reduction.set(s.get("noise_reduction", 0.0))
        self.app_state.v_film_grain.set(s.get("film_grain", 0))
        self.app_state.v_vignette.set(s.get("vignette", False))
        self.app_state.v_auto_action_enabled.set(s.get("auto_action_enabled", False))
        self.app_state.v_action_detector.set(s.get("action_detector", "person"))
        self.app_state.v_action_strength.set(s.get("action_strength", 0.65))
        self.app_state.v_action_auto_strength.set(s.get("action_auto_strength", False))
        self.app_state.v_action_smoothness.set(s.get("action_smoothness", 0.65))
        self.app_state.v_action_auto_smoothness.set(s.get("action_auto_smoothness", False))
        self.app_state.v_action_zoom_max.set(s.get("action_zoom_max", 2.0))
        self.app_state.v_action_padding.set(s.get("action_padding", 0.20))
        self.app_state.v_action_intro.set(s.get("action_intro", 1.5))
        self.app_state.v_action_bottom_crop.set(s.get("action_bottom_crop", 0.0))
        self.app_state.v_action_auto_bottom_crop.set(s.get("action_auto_bottom_crop", False))
        self.app_state.v_action_top_crop.set(s.get("action_top_crop", 0.0))
        self.app_state.v_action_auto_top_crop.set(s.get("action_auto_top_crop", False))
        self.app_state.v_action_vertical_bias.set(s.get("action_vertical_bias", 0.0))
        self.app_state.v_action_auto_vertical_bias.set(s.get("action_auto_vertical_bias", False))
        self.app_state.v_action_smart_auto_crop.set(s.get("action_smart_auto_crop", False))
        self.app_state.v_action_auto_pillarbox.set(s.get("action_auto_pillarbox", False))
        self.app_state.v_bg_sub_enable.set(s.get("bg_sub_enable", False))
        self.app_state.v_dmd_visibility_score_enabled.set(s.get("dmd_visibility_score_enabled", False)) # NEW
        self.app_state.v_dmd_readability_score_enabled.set(s.get("dmd_readability_score_enabled", True)) # NEW
        self.app_state.v_target_width.set(s.get("target_width", 128))
        self.app_state.v_target_height.set(s.get("target_height", 32))
        self.app_state.v_target_preset.set(s.get("target_preset", "128x32 (1x1)"))
        self.app_state.v_text_overlay_enabled.set(s.get("text_overlay_enabled", False))
        self.app_state.v_text_content.set(s.get("text_content", ""))
        self.app_state.v_text_font_size.set(s.get("text_font_size", 8))
        self.app_state.v_text_color.set(s.get("text_color", "white"))
        self.app_state.v_text_position.set(s.get("text_position", "bottom_center"))
        self.app_state.v_text_font_file.set(s.get("text_font_file", "HelvetiPixel.ttf"))
        self.app_state.v_text_style.set(s.get("text_style", "outline"))
        self.app_state.v_text_bg.set(s.get("text_bg", False))
        self.app_state.v_text_bg_opacity.set(s.get("text_bg_opacity", 60))
        self.app_state.v_text_animation.set(s.get("text_animation", "none"))
        self.app_state.v_max_dur_enabled.set(s.get("max_dur_enabled", True))
        self.app_state.v_max_duration.set(s.get("max_duration", 120.0))
        self.app_state.v_auto_color_enabled.set(s.get("auto_color_enabled", False))
    ...
    def _on_auto_color_toggle(self):
        ...
        """
        enabled = self.app_state.v_auto_color_enabled.get()
        ...
        if enabled:
            ...
            if self.app_state.v_mode.get() == "custom" and self._pre_auto_color_values:
                self.app_state.v_contrast.set(self._pre_auto_color_values["contrast"])
                self.app_state.v_saturation.set(self._pre_auto_color_values["saturation"])
                self.app_state.v_brightness.set(self._pre_auto_color_values["brightness"])
                self.app_state.v_gamma.set(self._pre_auto_color_values["gamma"])
                self.app_state.v_sharpen_lum.set(self._pre_auto_color_values["sharpen_lum"])
                self.app_state.v_sharpen_chr.set(self._pre_auto_color_values["sharpen_chr"])
            # ── Re-enable controls ─────────────────────────────────────────────
    ...
    def _refresh_auto_color_values(self, path: str):
        ...
        def _run():
            ...
            def _apply():
                ...
                if not self.app_state.v_auto_color_enabled.get():
                    return
                if ok and params:
                    # Update sliders only if already in custom mode
                    # (avoids re-packing layout and blocking Advanced Settings)
                    if self.app_state.v_mode.get() == "custom":
                        self.app_state.v_contrast.set(params["contrast"])
                        self.app_state.v_saturation.set(params["saturation"])
                        self.app_state.v_brightness.set(params["brightness"])
                        self.app_state.v_gamma.set(params["gamma"])
                        self.app_state.v_sharpen_lum.set(params.get("sharpen_lum", 1.8))
                        self.app_state.v_sharpen_chr.set(params.get("sharpen_chr", 0.5))
    ...
    def _collect_params(self):
        return {
            ...
            "mode":            self.app_state.v_mode.get(),
            "max_workers":     self.app_state.v_workers.get(),
            "scroll_speed":    self.app_state.v_scroll.get(),
            "bottom_crop_pct": self.app_state.v_bottom_crop.get(),
            "top_crop_pct":    self.app_state.v_top_crop.get(),
            "scroll_cycles":   self.app_state.v_scroll_cycles.get(),
            "fps_min":         self.app_state.v_fps_min.get(),
            "fps_max":         self.app_state.v_fps_max.get(),
            "contrast":        self.app_state.v_contrast.get(),
            ...
            "hue_shift":       self.app_state.v_hue_shift.get(),
            "noise_reduction": self.app_state.v_noise_reduction.get(),
            "film_grain":      int(self.app_state.v_film_grain.get()),
            "vignette":        self.app_state.v_vignette.get(),
            "auto_action_enabled": self.app_state.v_auto_action_enabled.get(),
            "action_detector": self.app_state.v_action_detector.get(),
            "action_strength": self.app_state.v_action_strength.get(),
            ...
            "target_width": self.app_state.v_target_width.get(),
            "target_height": self.app_state.v_target_height.get(),
            "text_overlay_enabled": self.app_state.v_text_overlay_enabled.get(), # Collect text overlay params
            "text_content": self.app_state.v_text_content.get(),
            "text_font_size": self.app_state.v_text_font_size.get(),
            "text_color": self.app_state.v_text_color.get(),
            "text_position": self.app_state.v_text_position.get(),
            ...
            "max_duration": self.app_state.v_max_duration.get() if self.app_state.v_max_dur_enabled.get() else 0.0,
            # auto-colorimetry
            "auto_color_enabled": self.app_state.v_auto_color_enabled.get(),
            # Log level
            ...
            } if self.app_state.v_let_me_handle_it.get() else {}
        )
