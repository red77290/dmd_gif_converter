...
class ActionsPanel(ctk.CTkFrame):
    ...
    def _build_actions_panel(self, parent):
        ...
        self.app_state.v_batch_auto_trash = tk.BooleanVar(value=True)
        ctk.CTkCheckBox(batch_cleanup_frame, text="Auto-Trash <=", variable=self.app_state.v_batch_auto_trash, width=10, checkbox_height=18, checkbox_width=18, font=ctk.CTkFont(size=11)).pack(side="left", padx=(4, 2))
        
        self.app_state.v_batch_trash_score = tk.StringVar(value="50")
        ctk.CTkEntry(batch_cleanup_frame, textvariable=self.app_state.v_batch_trash_score, width=35, height=20, font=ctk.CTkFont(size=11)).pack(side="left")
    ...
    def browse_output(self):
        ...
        if folder:
            self.app_state.v_output_dir.set(folder)
    ...
    def _run_tasks(self, tasks, params):
        ...
        per_gif_enabled = hasattr(self, 'v_per_gif_config') and self.app_state.v_per_gif_config.get()
    ...
    def _run_batch_folder(self, folder_in, folder_out, params):
        ...
        if self.app_state.v_batch_auto_trash.get():
            try:
                threshold = int(self.app_state.v_batch_trash_score.get())
