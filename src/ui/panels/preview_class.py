...
class PreviewPanel(ctk.CTkFrame):
    def _build_preview_area(self, parent):
        ...
        self._sl_start = ctk.CTkSlider(
            self._trim_frame, from_=0, to=1, variable=self.app_state.v_trim_start,
        ...
        self._sl_end = ctk.CTkSlider(
            self._trim_frame, from_=0, to=1, variable=self.app_state.v_trim_end,
    ...
    def _compute_led_sim_display_size(self):
        ...
        try:
            w = self.app_state.v_target_width.get()
            h = self.app_state.v_target_height.get()
        except Exception:
            w, h = 128, 32
        scale = LED_SIM_SCALE
    ...
    def _get_final_canvas_size(self):
        ...
        if getattr(self, "v_led_sim", None) and self.app_state.v_led_sim.get():
    ...
    def _update_dmd_canvas_size(self, *_):
        ...
        if hasattr(self, "_dmd_title_label"):
            sim_badge = "  💡" if (getattr(self, "v_led_sim", None) and self.app_state.v_led_sim.get()) else ""
    ...
    def _start_auto_generation(self, src):
        if self._auto_rendering:
            ...
            return
        if not self.app_state.v_auto_action_enabled.get():
            self._stop_auto_preview()
            self._draw_auto_canvas_idle()
        ...
        cfg = AutoActionConfig(
            detector=self.app_state.v_action_detector.get(),
            strength=float(self.app_state.v_action_strength.get()),
            smoothness=float(self.app_state.v_action_smoothness.get()),
            zoom_max=float(self.app_state.v_action_zoom_max.get()),
            padding=float(self.app_state.v_action_padding.get()),
            intro_duration=float(self.app_state.v_action_intro.get()),
            bg_sub_enable=self.app_state.v_bg_sub_enable.get(),
            bottom_crop_pct=float(self.app_state.v_action_bottom_crop.get()),
            auto_bottom_crop=bool(self.app_state.v_action_auto_bottom_crop.get()),
            top_crop_pct=float(self.app_state.v_action_top_crop.get()),
            auto_top_crop=bool(self.app_state.v_action_auto_top_crop.get()),
            vertical_bias=float(self.app_state.v_action_vertical_bias.get()),
            auto_vertical_bias=bool(self.app_state.v_action_auto_vertical_bias.get()),
            smart_auto_crop=bool(self.app_state.v_action_smart_auto_crop.get()),
            auto_pillarbox_crop=bool(self.app_state.v_action_auto_pillarbox.get()),
            dmd_visibility_score_enabled=bool(self.app_state.v_dmd_visibility_score_enabled.get()), # NEW
            dmd_readability_score_enabled=bool(self.app_state.v_dmd_readability_score_enabled.get()), # NEW
            start_s=start_s,
            end_s=end_s,
    ...
    def _start_dmd_generation(self, src, is_already_converted=False):
        ...
        start_s, end_s = self._get_trim()
        led_sim = self.app_state.v_led_sim.get()
        if led_sim:
    ...
    def _on_dmd_ready(self, pil_frames, delays, tmpdir, out_gif):
        try:
            ...
            self._dmd_info.configure(
                text=f"✅  {self.app_state.v_target_width.get()}×{self.app_state.v_target_height.get()}  ·  {len(pil_frames)} frames  ·  {size_kb} KB"
            )
            self._animate_dmd()
            # Start the pending render (if any new settings changed while we were busy)
    ...
    def _on_max_dur_toggle(self):
        ...
        enabled = self.app_state.v_max_dur_enabled.get()
    ...
    def _apply_max_duration(self):
        ...
        if not self.app_state.v_max_dur_enabled.get():
            return
