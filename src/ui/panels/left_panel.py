...
class LeftPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self._status_lbl = ctk.CTkLabel(
            lp, text="Ready", text_color="#556677", font=ctk.CTkFont(size=10)
        )
        self._status_lbl.grid(row=6, column=0, padx=8, pady=(0, 2), sticky="w")

        self._progress = ctk.CTkProgressBar(lp, height=6)
        self._progress.set(0)
        self._progress.grid(row=7, column=0, padx=8, pady=(0, 6), sticky="ew")
    ...
    def _build_search_section(self, parent):
        ...
        self._ratio_menu = ctk.CTkOptionMenu(
            dim_f, variable=self.app_state.v_search_ratio,
    ...
    def _update_per_gif_status(self, path: str, saved: bool = False):
        ...
        if hasattr(self, "_per_gif_status_lbl"):
            name = Path(path).name
            if saved:
                self._per_gif_status_lbl.configure(text=f"✅ config: {name[:20]}")
            else:
                self._per_gif_status_lbl.configure(text=f"(default) {name[:20]}")
